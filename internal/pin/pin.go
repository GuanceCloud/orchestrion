...
const (
	orchestrionImportPath       = "github.com/GuanceCloud/orchestrion"
	legacyOrchestrionImportPath = "github.com/DataDog/orchestrion"
	legacyTracerV2Path          = "github.com/DataDog/dd-trace-go/v2"
	legacyTracerV2AllPath       = "github.com/DataDog/dd-trace-go/orchestrion/all/v2"
)
...
func PinOrchestrion(ctx context.Context, opts Options) error {
	...
	// Remove dependencies left by the upstream DataDog pin before resolving the
	// GuanceCloud integration graph. Keeping both namespaces can enable duplicate
	// integrations and preserve obsolete customer-side replace directives.
	if err := gomod.RunEdit(ctx, goMod, legacyModuleEdits(curMod)...); err != nil {
		return fmt.Errorf("migrating legacy module entries in %q: %w", goMod, err)
	}
...
func updateToolFile(file *dst.File) (*importSet, error) {
	...
	// GuanceCloud is a standalone module namespace. Remove imports generated by
	// the upstream DataDog pin so the project cannot load both integration sets.
	importSet.Remove(legacyOrchestrionImportPath)
	importSet.Remove(legacyOrchestrionImportPath + "/instrument")
	importSet.Remove(legacyTracerV2AllPath)

	return importSet, nil
}

func legacyModuleEdits(mod gomod.File) []gomod.Edit {
	var edits []gomod.Edit
	for _, req := range mod.Require {
		switch req.Path {
		case legacyOrchestrionImportPath, legacyTracerV2AllPath:
			edits = append(edits, gomod.DropRequire(req.Path))
		}
	}
	for _, replacement := range mod.Replace {
		path := replacement.Old.Path
		if path == legacyOrchestrionImportPath || path == legacyTracerV2Path || strings.HasPrefix(path, "github.com/DataDog/dd-trace-go/") {
			edits = append(edits, gomod.DropReplace{Path: path, Version: replacement.Old.Version})
		}
	}
	return edits
}
...
func updateGoGenerateDirective(opts Options, file *dst.File) {
	...
	const legacyPrefix = "//go:generate go run github.com/DataDog/orchestrion pin"
		...
		dstNodeVisitor(func(node dst.Node) bool {
			...
			case *dst.File, dst.Decl:
				...
				for i, dec := range decs.Start {
					if dec != prefix && !strings.HasPrefix(dec, prefix+" ") && dec != legacyPrefix && !strings.HasPrefix(dec, legacyPrefix+" ") {
						continue
					}
					if found {
						decs.Start[i] = ""
